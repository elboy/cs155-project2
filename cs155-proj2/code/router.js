...
router.post('/post_register', asyncMiddleware(async (req, res, next) => {
  ...
  // Example of correct SQL prepared statements
...
router.get('/close', asyncMiddleware(async (req, res, next) => {
  ...
  const db = await dbPromise;
  /* EXPLOIT Echo, deleted lines 
  const query = `DELETE FROM Users WHERE username == "${req.session.account.username}";`;
  await db.get(query);
   */

  /* EXPLOIT Echo, added line */
  const query = `DELETE FROM Users WHERE username == ?;`;
  /* EXPLOIT Echo, added line */
  const result = await db.run(query, [req.session.account.username]);
...
router.get('/profile', asyncMiddleware(async (req, res, next) => {
  ...
  if(req.query.username != null) { // if visitor makes a search query
    ...
    let result;
    try {
      result = await db.run(query, req.query.username);
    } catch(err) {
      result = false;
    }
    */
    ...
    if(result) { // if user exists
      ...
      // TODO: Make it so username appears in the UI (escaping, e.g.)
      render(req, res, next, 'profile/view', 'View Profile', "User does not exist!", req.session.account);
    }
...
router.get('/transfer', (req, res, next) => {
  ...
  /* EXPLOIT Beta, random key for generating auth_token */
  var random_key = generateRandomness();
  // TODO: Is this the right value of data?
  var auth_token = HMAC(random_key, req.session.account);
  /* EXPLOIT Beta, set server session auth_token */
  req.session.auth_token = auth_token;
  render(req, res, next, 'transfer/form', 'Transfer Bitbars', false, {receiver:null, amount:null, auth_token: auth_token});
});
...
router.post('/post_transfer', asyncMiddleware(async(req, res, next) => {
  ...
  if(req.body.destination_username === req.session.account.username) {
    /* EXPLOIT Beta, make sure page is refreshed with same auth_token */
    render(req, res, next, 'transfer/form', 'Transfer Bitbars', 'You cannot send money to yourself!', {receiver:null, amount:null, auth_token: req.body.authenticity_token});
    return;
  }

  /* EXPLOIT Beta, ensure correct auth_token */
  if(req.body.authenticity_token !== req.session.auth_token) {
    /* EXPLOIT Beta, make sure page is refreshed with same auth_token */
    render(req, res, next, 'transfer/form', 'Transfer Bitbars', 'Invalid authenticity token', {receiver:null, amount:null, auth_token: req.body.authenticity_token});
    return;
  }
  ...
  if(receiver) { // if user exists
    ...
    if(Number.isNaN(amount) || amount > req.session.account.bitbars || amount < 1) {
      /* EXPLOIT Beta, make sure page is refreshed with same auth_token */
      render(req, res, next, 'transfer/form', 'Transfer Bitbars', 'Invalid transfer amount!', {receiver:null, amount:null, auth_token: req.body.authenticity_token});
      return;
    }
    ...
    await db.exec(query);
    /* EXPLOIT Beta, make sure page is refreshed with same auth_token */
    render(req, res, next, 'transfer/success', 'Transfer Complete', false, {receiver, amount, auth_token: req.body.authenticity_token});
  } else { // user does not exist
    render(req, res, next, 'transfer/form', 'Transfer Bitbars', 'This user does not exist!', {receiver:null, amount:null, auth_token: req.body.authenticity_token});
  }
}));
